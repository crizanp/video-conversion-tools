...
export default function WebmToMp4Converter() {
    ...
    const handleFileSelect = (event) => {
        ...
        if (validFiles.length === 0) {
            setStatus('Please select valid WEBM video files.');
            return;
        }
    ...
    // Filter files to ensure only WEBM files are selected
    ...
    const handleDrop = (e) => {
        ...
        if (validFiles.length === 0) {
            setStatus('Please drop valid WEBM video files.');
            return;
        }
    ...
    const performConversion = async () => {
        if (selectedFiles.length === 0) {
            setStatus('Please select WEBM files to convert.');
            return;
        }
        ...
        try {
            ...
            for (let i = 0; i < totalFiles; i++) {
                ...
                try {
                    ...
                    const fileName = file.name.replace(/\.webm$/i, '.mp4');
    ...
    const featuresData = [
        {
            ...
            description: "Convert WEBM files to MP4 in seconds with blazing-fast speed and zero quality loss.",
        },
        {
            icon: (
                <svg className="w-8 h-8 text-white" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
        ...
        },
        {
            ...
            description: "Upload and convert multiple WEBM files at once — batch processing made easy.",
        },
    ];
                                ...
                                <div className="flex items-start">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-indigo-600 mb-4">
                                            Your WEBM files have been successfully converted to MP4 format.
                                        </p>
